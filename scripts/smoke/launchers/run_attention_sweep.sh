...
#     nohup bash scripts/run_attention_sweep.sh > experiments/attncomm_sweep/sweep.log 2>&1 &
#
# Output layout:
#     experiments/attncomm_sweep/msg{D}_h{H}_seed0/final.pt
#
# Expected runtime:
...
OUT_BASE="$REPO_ROOT/experiments/attncomm_sweep"
mkdir -p "$OUT_BASE"
...
for D in "${MSG_DIMS[@]}"; do
    for H in "${NUM_HEADS[@]}"; do
        ...
        # The trainer's save path is hard-coded to experiments/attncomm_mappo/seed{N},
