...
for COND in "${CONDITIONS[@]}"; do
    BASE_DIR="$REPO_ROOT/experiments/$COND"
    row="$COND"
...
# condition folder; the FC/attn and GNN runners write at experiments/ root.
for LOGPATH in \
    "$REPO_ROOT/experiments/smacv2_nocomm/3seeds.log" \
    "$REPO_ROOT/experiments/smacv2_nocomm_3seeds.log" \
    "$REPO_ROOT/experiments/smacv2_fc_attn_6seeds.log" \
    "$REPO_ROOT/experiments/smacv2_gnn_3seeds.log"; do
    if [ -f "$LOGPATH" ]; then
        echo
