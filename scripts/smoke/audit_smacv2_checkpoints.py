"""SMACv2 checkpoint field-completeness auditor.

Loads every `artifacts/checkpoints/smacv2_*/seed*/final.pt` (and, if present, the
resumable `latest.pt`) and confirms each checkpoint contains the fields the
current trainer writes. Catches the case where a run was interrupted and a
checkpoint ended up incomplete.

This is READ-ONLY. Safe to run any time, including against runs that are still
training (it will just see fewer checkpoints).

WHY THIS FILE WAS REWRITTEN
---------------------------
An earlier version of this auditor required the fields `pipeline`,
`final_win_rate_20` and `action_valid_frac`, and looked in fixed directories
named `smacv2_nocomm`, `smacv2_fccomm`, etc. None of those match what
14_mappo_smacv2_matched.py actually produces, so it reported every good
checkpoint as BAD. It now checks the real keys and discovers experiment
directories by globbing, so it works with any mode/condition combination
(smacv2_standard_none, smacv2_epo_attn, ...).

The trainer writes two files per seed:
  final.pt   - compact results: policy, critic, args, steps,
               final_test_win_rate, final_test_return, final_train_win_rate
  latest.pt  - full resumable checkpoint: adds opt_a, opt_c, obs_norm,
               state_norm and RNG states

USAGE
-----
    python scripts/smoke/audit_smacv2_checkpoints.py
    python scripts/smoke/audit_smacv2_checkpoints.py --experiments-dir /path/to/experiments
    python scripts/smoke/audit_smacv2_checkpoints.py --check-latest   # also audit latest.pt
"""
    ...
    for f in unit_range_fields:
        if f in ckpt and ckpt[f] is not None:
            try:
                ...
                continue
            # NaN is allowed: evaluation may be disabled (--eval-episodes 0),
            # which legitimately leaves win rate as NaN. Only flag real numbers
...
def find_experiments_dir(cli_value: str | None) -> Path | None:
    ...
    # Walk up looking for a repo that has an artifacts/checkpoints/ dir or the trainer.
...
def main() -> int:
    ...
    parser.add_argument("--experiments-dir", default=None,
                        help="Path to artifacts/checkpoints/ dir. Auto-detected if omitted.")
    ...
    if experiments_dir is None:
        print("ERROR: cannot locate artifacts/checkpoints/ dir. Pass --experiments-dir.",
              file=sys.stderr)
        return 1
    ...
    # Discover experiment directories by glob rather than a fixed list, so this
    # works for any mode/condition: smacv2_standard_none, smacv2_epo_attn, ...
