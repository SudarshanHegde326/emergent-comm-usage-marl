...
echo
# FIX 2: detect whether we are inside an active virtualenv, so the pip
# command can be chosen correctly (no --break-system-packages in a venv).
...
# FIX 2: only use --break-system-packages when NOT in a venv.
# ---------------------------------------------------------------
...
# FIX 3: a directory existing is not enough — verify it actually
# contains the Versions/ subdir that marks a real SC2 install.
# ---------------------------------------------------------------
...
# FIX 4: SMACv2 maps are a SEPARATE download from the base game.
# A missing map pack causes a 'map not found' error in Check 4 that
# looks like an env bug but is not. Verify the Maps dir here.
# This is a WARN (not a hard FAIL) so the rest of the checks still run.
# ---------------------------------------------------------------
...
# FIX 5: SMACv2 (unlike v1) generally requires a procedural
# `capability_config` in addition to map_name. This script tries the
# documented v2 construction; if your installed SMACv2 needs a
# different config shape, the EXCEPTION printed here tells you so —
# confirm the exact constructor in the Ellis 2023 paper / README.
# ---------------------------------------------------------------
