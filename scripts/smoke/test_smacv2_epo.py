...
def make_env(epo: bool, seed: int = 0):
    """Mirror the trainer's make_env(): EPO is prob_obs_enemy=0 + action_mask=False.

    conic_fov is False in both modes. Standard is 5v5; the paper's EPO challenge
    is 6v5, so we match that.
    """
...
def mean_available_actions(env, n_agents: int, steps: int, rng: random.Random) -> float:
    """Average number of 'available' actions per living agent over a rollout.

    Under EPO (action_mask=False) this should be HIGHER than under standard,
    because attack actions are no longer gated by whether the enemy is in range.
    """
    env.reset()
    counts = []
...
def main() -> int:
    ...
    # The defining EPO mechanism: removing the action mask means more actions
    # read as available (attacks are no longer range-gated). If this does not
    # hold, the EPO switches are not taking effect.
    checks = []
