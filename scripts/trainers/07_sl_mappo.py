"""Unified MAPPO trainer for Simple Speaker-Listener.

Three comm conditions in one file, selected by ``--comm-type``:
  - ``none``  : actor consumes padded obs only (no inter-agent msg).
  - ``fc``    : MessageHead + FCAggregator + actor on [obs, agg_msg].
  - ``attn``  : MessageHead + MultiHeadAttentionAggregator + actor on [obs, agg_msg].

Same MAPPO recipe as the Simple Spread trainers, with three SL-specific
adaptations:
  1. Padded obs (SL_PADDED_OBS_DIM=13) via ``pad_obs_to_max``.
  2. Action sampling uses ``action_mask`` to zero out invalid actions.
  3. ``action_valid_frac`` logged per rollout (must stay at 1.0).

USAGE
-----
    # Smoke
    python 07_sl_mappo.py --comm-type none --seed 999 --total-steps 2000 --no-wandb
    python 07_sl_mappo.py --comm-type fc   --seed 999 --total-steps 2000 --no-wandb
    python 07_sl_mappo.py --comm-type attn --seed 999 --total-steps 2000 --no-wandb

    # Full run (200k steps)
    python 07_sl_mappo.py --comm-type attn --seed 0 --total-steps 200000

OUTPUTS
-------
- wandb run (unless --no-wandb)
- artifacts/checkpoints/sl_<comm-type>/seed{N}/final.pt
"""
...
from src.comms.gnn_comm import GNNAggregator# noqa: E402
...
class SLPipelineNone(nn.Module):
    """No-comm baseline: actor on padded obs only.

    For uniformity with the other two variants, the forward returns
    `(logits, None, None)` so the trainer can call `pipeline(...)`
    with a single unpack shape.
    """
    ...
    def forward(self, obs_NB):
        ...
        logits = self.actor(obs_NB)            # (B, N, action_dim)
        return logits, None, None


class SLPipelineFC(nn.Module):
    """FC-comm: MessageHead + FCAggregator + CommActor."""
    ...
    def forward(self, obs_NB):
        msgs = self.msg_head(obs_NB)           # (B, N, msg_dim)
        agg = self.agg(msgs)                   # (B, N, msg_dim)
        logits = self.actor(obs_NB, agg)       # (B, N, action_dim)
        return logits, msgs, agg


class SLPipelineAttn(nn.Module):
    """Attention-comm: MessageHead + MultiHeadAttention + CommActor."""
...
class SLPipelineGNN(nn.Module):
    """GNN-comm: MessageHead + GNNAggregator + CommActor."""
    ...
    def forward(self, obs_NB):
        msgs = self.msg_head(obs_NB)           # (B, N, msg_dim)
        agg, _per_layer = self.agg(msgs)       # (B, N, msg_dim)
        logits = self.actor(obs_NB, agg)       # (B, N, action_dim)
        return logits, msgs, agg
    ...
    while total_env_steps < args.total_steps:
        ...
        while not buf.is_full():
            ...
            with torch.no_grad():
                ...
                # Apply action mask BEFORE Categorical so invalid actions
                # have prob ~ 0.
            ...
            # Sanity: actions must all be within each agent's valid range
