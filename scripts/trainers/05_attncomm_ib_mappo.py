"""MAPPO training with attention-comm routed through an Information Bottleneck."""
...
class IBAttentionPipeline(nn.Module):
    def __init__(self, obs_dim: int, msg_dim: int, action_dim: int,
                 num_heads: int = 2, hidden: int = 128, attn_dropout: float = 0.0) -> None:
        super().__init__()
        self.msg_head = MessageHead(obs_dim, msg_dim, hidden=hidden // 2)
        self.channel = StochasticBottleneckChannel(msg_dim=msg_dim)
...
def train(args):
    ...
    print(f"[ib-attn/train] device={device} seed={args.seed} beta={args.beta}")
    ...
    pipeline = IBAttentionPipeline(
        obs_dim=obs_dim, msg_dim=args.msg_dim, action_dim=action_dim,
        num_heads=args.num_heads, hidden=args.hidden, attn_dropout=args.attn_dropout
    ).to(device)
    ...
    while total_env_steps < args.total_steps:
        ...
        for _ in range(args.ppo_epochs):
            ...
            for start in range(0, B_total, args.minibatch_size):
                ...
                # Active reparameterization backpropagation target loss query
                kl_term = pipeline.channel.kl_loss()
                last_kl_logged = float(kl_term.item())
                ...
                # Augment the objective loss with the statistical KL information penalty
        ...
        print(f"steps={total_env_steps:>8d}  mean_ret={mean_ret:7.2f}  fps={fps}  |msg_pre|={msg_pre_l2:.3f}  |z|={z_l2:.3f}  kl_mean={last_kl_logged:.3f}")

    beta_tag = str(args.beta).replace(".", "p")
    save_dir = REPO_ROOT / "experiments" / f"attncomm_ib_b{beta_tag}" / f"seed{args.seed}"
    save_dir.mkdir(parents=True, exist_ok=True)
    torch.save({"pipeline": pipeline.state_dict(), "critic": critic.state_dict(), "args": vars(args)}, save_dir / "final.pt")
    print(f"[ib-attn/train] saved -> {save_dir}/final.pt")
