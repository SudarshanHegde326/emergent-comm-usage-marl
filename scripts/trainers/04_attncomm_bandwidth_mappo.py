"""MAPPO with attention-comm + a BANDWIDTH-LIMITED channel on the messages."""
...
class BandwidthAttentionPipeline(nn.Module):
    """End-to-end attention pipeline with a CHANNEL between msg head & agg."""
    ...
    def __init__(
        ...
        num_heads: int = 2,
        hidden: int = 128,
        channel_type: str = "low-rank",
        bandwidth: int = 8,
        attn_dropout: float = 0.0,
    ) -> None:
        super().__init__()
        self.msg_head = MessageHead(obs_dim, msg_dim, hidden=hidden // 2)
        self.channel = build_channel(channel_type, msg_dim=msg_dim, bandwidth=bandwidth)
    ...
    def forward(self, obs_NB: torch.Tensor):
        msgs_pre = self.msg_head(obs_NB)            # (B, N, D)
        msgs_post = self.channel(msgs_pre)          # (B, N, D)
        agg, weights = self.agg(msgs_post)
        logits = self.actor(obs_NB, agg)
        return logits, msgs_pre, msgs_post, agg, weights
...
class RolloutBuffer:
    def __init__(self, T, N, obs_dim, msg_dim, num_heads, device):
        ...
        self.msgs_post = torch.zeros(T, N, msg_dim, device=device)
    ...
    def add(self, obs, global_obs, actions, logprobs, value, reward, done,
            msgs_pre, msgs_post, attn_weights):
        i = self.ptr
        self.obs[i] = obs
...
def train(args):
    ...
    print(f"[bw-attn/train] device={device} seed={args.seed} channel={args.channel_type}")
    ...
    if args.no_wandb:
        ...
        wandb_run = wandb.init(
            ...
            name=f"attncomm_bw{args.bandwidth}_{args.channel_type}_seed{args.seed}",
            config=vars(args),
        )
    ...
    while total_env_steps < args.total_steps:
        ...
        while not buf.is_full():
            ...
            with torch.no_grad():
                logits, msgs_pre, msgs_post, _, weights = pipeline(obs_t.unsqueeze(0))
                logits = logits.squeeze(0)
                msgs_pre_s = msgs_pre.squeeze(0)
                msgs_post_s = msgs_post.squeeze(0)
            ...
            buf.add(obs_t, global_obs, actions, logprobs, value, team_reward, terminated,
                    msgs_pre_s, msgs_post_s, weights_s)
        ...
        with torch.no_grad():
            ...
            msg_post_l2 = buf.msgs_post.norm(dim=-1).mean().item()
            attn_ent = attention_entropy(buf.attn_weights).item()

        print(f"steps={total_env_steps:>8d}  mean_ret={mean_ret:7.2f}  fps={fps}  |msg_pre|={msg_pre_l2:.3f}  bw_l2={msg_post_l2:.3f}")
        if wandb_run is not None:
            wandb_run.log({"step": total_env_steps, "mean_return_last20": mean_ret})

    save_dir = REPO_ROOT / "experiments" / f"attncomm_bw{args.bandwidth}" / f"seed{args.seed}"
    save_dir.mkdir(parents=True, exist_ok=True)
    torch.save({"pipeline": pipeline.state_dict(), "critic": critic.state_dict(), "args": vars(args)}, save_dir / "final.pt")
    print(f"[bw-attn/train] saved -> {save_dir}/final.pt")
...
def build_argparser():
    ...
    p.add_argument("--channel-type", type=str, default="low-rank", choices=["none", "low-rank", "quantized"])
