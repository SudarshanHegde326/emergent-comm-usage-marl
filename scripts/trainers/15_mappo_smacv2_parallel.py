"""Parallel (vectorized) MAPPO for SMACv2 — the high-throughput trainer.

This runs N StarCraft II games at once (via vec_smacv2.SubprocVecSMAC), so the
GPU is fed a batch of N environments every step instead of waiting on one. It
reuses the PROVEN components from 14_mappo_smacv2_matched.py unchanged — the
policy, critic, all communication modules, the running normaliser, GAE, greedy
evaluation, and the checkpoint format — and only changes the DATA FLOW to carry
an environment-batch dimension B = num_envs.

WHY A SEPARATE FILE
-------------------
The single-env trainer (14_...) is verified by 47 tests and has completed real
300k runs. It stays intact as a fallback. This file is the parallel variant; if
a batching bug is found under deadline, the working trainer is untouched.

THE DANGEROUS PARTS (and how each is handled)
---------------------------------------------
Parallelism corrupts results silently if any of these is wrong. Each is called
out at its site below and covered by tests in tests/test_parallel_trainer.py:

  1. Per-ENV episode boundaries. GAE and the recurrent state must reset for
     env b exactly when env b's episode ends — never bleeding one env's return
     or hidden state into another's. dones are shape (T, B); GAE runs per-env.
  2. Per-ENV, per-AGENT recurrent hidden state, shape (B*N, hidden), reset only
     for the envs that finished this step.
  3. The vec wrapper AUTO-RESETS on natural termination and returns the next
     episode's first obs. The stored obs at step t is the one ACTED on; the
     done flag marks that the NEXT stored obs is a fresh episode. This matches
     the standard vectorized-RL contract and the single-env done handling.
  4. Time-limit truncation is tracked HERE (the trainer knows ep_limit); the
     worker only signals natural termination. Both end the return bootstrap.
  5. Normaliser statistics are frozen during the update and refreshed after,
     exactly as in the single-env trainer, so the PPO ratio is consistent.

Throughput features:
  * torch.no_grad() around all rollout inference (autograd off during collect).
  * Optional AMP (--amp) for the PPO update. NOTE: your bottleneck is the CPU
    (StarCraft), not the GPU, so AMP will give little here — it is provided for
    completeness, off by default.
  * One batched host->device transfer per step instead of one per env.

USAGE
-----
  python scripts/trainers/15_mappo_smacv2_parallel.py \
      --comm attn --mode epo --seed 0 --num-envs 8 --total-steps 3000000

All flags from 14_... are accepted (comm upgrades, aux loss, checkpoint/resume,
comm-to-memory, eval, etc.). --num-envs and --amp are new.
"""
...
# Must be a real import (not importlib under a synthetic name), so that 'spawn'
# workers created by the vec wrapper can re-import modules to unpickle.
_HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(_HERE))
import vec_smacv2 as vec                              # noqa: E402

# 14_... starts with a digit, so import it via importlib but register it under a
# clean name in sys.modules so any pickling references resolve.
...
def compute_gae_batched(rewards, values, dones, truncs, last_values, gamma, lam):
    """Bootstrap on truncation; stop only on true termination.

    dones:  episode ended (terminated OR truncated)
    truncs: time-limit truncation only
    true terminal = done & not truncated
    """
    T, B = rewards.shape
...
def build_args(argv=None):
    p = S.build_parser()
    # New flags for the parallel trainer.
    ...
    # With B parallel envs, one "step" advances B env-steps. rollout_len is
    # counted in ENV-STEPS-PER-ENV, so the data gathered per rollout is
...
def train_parallel(args, env_factory_override=None):
    ...
    # We need ep_limit to configure worker-side truncation, but ep_limit comes
    # from get_env_info(), which needs a built env. So build once to read info,
    # then rebuild with ep_limit. Cheap: it's a one-time setup cost.
    ...
    # enemy-feature slice for the auxiliary sighting label
    enemy_slice = None
    ...
    # Version-compatible GradScaler. Newer PyTorch exposes torch.amp.GradScaler;
    # older cluster builds only have torch.cuda.amp.GradScaler. When AMP is off
    # (the normal case here -- the bottleneck is StarCraft on CPU, not the GPU),
    # we avoid the AMP API entirely and use a tiny no-op scaler, so the trainer
    # runs identically on any PyTorch version.
    class _NullScaler:
    ...
    out = Path(args.out_dir or
               f"artifacts/checkpoints/par_smacv2_{args.mode}_{args.comm}") / f"seed{args.seed}"
    ...
    def read_batch(obs_np, state_np, avail_np):
        """numpy (B,N,raw),(B,state),(B,N,A) -> device tensors + alive + sight.

        obs gets the agent-ID one-hot appended per agent (broadcast over B).
        """
    ...
    steps = 0                       # counts ENV-STEPS-PER-ENV (so one rollout
                                    # of length T advances `steps` by T, while
                                    # gathering T*B transitions)
    ...
    # ckpt_interval / eval_interval are counted in TOTAL TRANSITIONS, matching
    # total_steps (which the loop compares as steps * B). steps itself is per-env.
    ...
    while steps * B < args.total_steps:
        ...
        for i in range(T):
            ...
            # `done_env` (B,) already covers BOTH natural termination and
            # time-limit truncation (the worker enforces ep_limit and auto-
            # resets in either case), so the returned obs is ALWAYS the first
            # obs of a fresh episode wherever done_env is True. The parent does
            # no mid-rollout reset -- that would perturb the other envs.
        ...
        for _ in range(args.ppo_epochs):
            ...
            for c in chunk_ids:
                ...
                # autocast only under CUDA+AMP; a plain nullcontext on CPU or
                # when AMP is off. Version-compatible: fall back to
                # torch.cuda.amp.autocast on older PyTorch.
                if use_amp:
                    try:
                ...
                with _amp_ctx:
                    ...
                    for t in range(s0, s1):
                        ...
                        vals_seq.append(v)
                        # per-ENV recurrent reset at episode boundaries
                        done_t = buf["done"][t]                        # (B,)
                        if done_t.any():
                            m = (done_t > 0.5)
