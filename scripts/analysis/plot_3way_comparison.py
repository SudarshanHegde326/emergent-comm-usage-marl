...
def main():
    ...
    # Load all aggregate summary files
    data = []
    ...
    # Plot Bars matching our standardized configuration color definitions
    ...
    # Individual per-seed dots overlaid to show data distribution density
    for i, seed_means in enumerate(per_seed):
        if len(seed_means) > 0:
            ax.scatter([i] * len(seed_means), seed_means,
                       color="black", alpha=0.55, s=22, zorder=3)

    # Statistical annotations: mean ± std value markers and tracking sample n
