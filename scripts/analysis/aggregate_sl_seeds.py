...
-----
    python aggregate_sl_seeds.py \
        --condition sl-attn \
        --checkpoints artifacts/checkpoints/sl_attn/seed0/final.pt \
                      artifacts/checkpoints/sl_attn/seed1/final.pt \
                      artifacts/checkpoints/sl_attn/seed2/final.pt \
        --eval-episodes 20 \
        --out artifacts/checkpoints/sl_attn/aggregate.json
"""
...
def eval_one_seed(checkpoint_path: Path, n_episodes: int, seed_offset: int,
    ...
    """Load a SL checkpoint, run `n_episodes` greedy eval episodes,
    return per-episode returns."""
