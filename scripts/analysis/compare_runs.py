...
def parse_log(file: Path) -> tuple[np.ndarray, np.ndarray]:
    """Parses standard training console output lines to extract training step trajectories."""
    # Matches both "global_step" format and "Steps=" format variations
...
def aggregate_seeds(files: list[Path]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Interpolates multiple random seed log timelines onto a uniform grid map to compute variance."""
...
def main():
    ...
    # Parse and plot data lines iteratively
